...
export default function Checkout({ selectedProducts }: Props) {
  ...
  const [taxRate] = useState<number>(0.03); // 9% tax rate
  const [logisticsRate] = useState<number>(0.04); // 2% logistics fee
  const [shippingCostPerProduct] = useState<number>(50.0); // $50 per product for shipping
  const [discountRate] = useState<number>(0.01); // 2% discount
  ...
  return (
    <>
      <div className="mx-auto mt-6 w-full flex-1 space-y-6 lg:mt-0 lg:w-full lg:max-w-4xl">
          ...
          <div className="space-y-2">
            ...
            {productsToRender.length > 0 ? (
              <div className="space-y-1">
                <dl className="flex items-center justify-between gap-4 border-t border-gray-200">
